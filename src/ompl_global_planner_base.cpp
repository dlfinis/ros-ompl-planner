...
namespace ompl_global_planner_base {
  ...
  void OmplGlobalPlannerBase::initialize(std::string name,
    ...
    // dynamic reconfigure
    dsrv_ = new dynamic_reconfigure::Server<PlannerConfig>(private_nh);
    dynamic_reconfigure::Server<PlannerConfig>::CallbackType cb_;
  ...
  void OmplGlobalPlannerBase::reconfigureCB(PlannerConfig& config,
                                        uint32_t level) {
  ...
  bool OmplGlobalPlannerBase::makePlan(
    ...
    if (ompl_simple_setup_->solve(max_planning_time_)) {
      ...
      if (map_resolution_ > 0.0) {
       ...
       ROS_INFO("Ompl states to ros pose stamped!");
